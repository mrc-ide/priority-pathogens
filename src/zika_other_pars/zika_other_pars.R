...
orderly_shared_resource("zika_functions.R" = "zika_functions.R",
                        "lassa_functions.R" = "lassa_functions.R"
                        )
...
mod_clean <- readRDS("models_curated.rds")
outb_clean <- readRDS("outbreaks_curated.rds")

# 10318 %in% unique(mod_clean$covidence_id)
# 10318 %in% unique(outb_clean$covidence_id)
# 10318 %in% unique(params_clean$covidence_id)
# filter(params_clean, covidence_id == 10318)$parameter_type
...
#
df_severity2 <- df_severity2 %>% filter(!covidence_id == 10318) #shouldn't have been extracted
metaanalysis_symp <- metaprop_wrap(dataframe = df_severity2 %>% filter(!qa_score<0.5),
              plot_pooled = TRUE, subgroup = NA,
              sort_by_subg = FALSE, xlabel = "Severity - proportion of symptomatic cases",
...
funnel_symp_meta <- funnel(metaanalysis_symp$result, common = FALSE,#only plots funnel for either common or random for some reason
                         pch = 22, bg = "dodgerblue3", level = 0.95)#,
# studlab = TRUE, cex.studlab = 0.75, pos.studlab = 2)
dev.off()
gg <- png::readPNG("temp.png", native = TRUE)
file.remove("temp.png")
funnel_symp_meta <- wrap_elements(plot = rasterGrob(gg, interpolate = TRUE))
ggsave("sympt_funnel.pdf", plot = funnel_symp_meta, width = 8, height = 6)
