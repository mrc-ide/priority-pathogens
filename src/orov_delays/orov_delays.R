...
delays %>% group_by(parameter_statistical) %>% summarise(length(parameter_statistical))
 
delays %>% group_by(delay_label) %>% summarise(length(delay_label))

## delays table 

delays %>% select(delay_label,parameter_value,
                  parameter_lower_bound,parameter_upper_bound,
                  parameter_unit,parameter_statistical,
                  parameter_uncertainty_single_value,
                  parameter_uncertainty_singe_type,
                  method_disaggregated_only,population_sample_type,
                  population_group,population_sample_size,
                  population_country,
                  population_study_start_year,
                  population_study_end_year) %>% group_by(delay_label) %>%
  mutate(delay_n = length(delay_label)) %>% arrange(-delay_n) %>% View()
