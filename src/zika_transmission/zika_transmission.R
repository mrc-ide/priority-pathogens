...
orderly_shared_resource("lassa_functions.R" = "lassa_functions.R",
                          "zika_functions.R" = 'zika_functions.R')
...
p1 <- forest_plot_zika(d1 |> filter(qa_score>0.5),expression(Evolutionary~Rate~(s/s/y ~10^{-4})),"genome_site",c(5,50)) + guides(color = guide_legend(title = "Segment", order = 1))
p2 <- forest_plot_zika(d2 |> filter(qa_score>0.5),expression(Substitution~Rate~(s/s/y ~10^{-4})),"genome_site",c(0,100)) + guides(color = guide_legend(title = "Segment", order = 1))
...
#
# p3 <- forest_plot_zika( d3_tmp, 'Overdispersion',"method_moment_value",c(-0.1,1.5))      # Add location to plot??


# for plotting need to separate groups!
p4_gp  <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(population_group=="General population"),'Attack Rate (%) [General Population]',"arate",c(-5,30))
p4_hcw <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(population_group=="Healthcare workers"),'Attack Rate (%) [HCW]',"arate",c(-5,100))
p4_pui <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(population_group=="Persons under investigation"),'Attack Rate (%) [Persons under investigation]',"arate",c(-5,100))
#p4_mix <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(population_group=="Mixed groups"),'Attack Rate (%)',"arate",c(-5,100))

#p4_gp + p4_hcw + p4_pui + p4_mix + plot_annotation(tag_levels = 'A')
p4_gp + p4_hcw + p4_pui + plot_annotation(tag_levels = 'A')

p4_primary   <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(arate=='Primary'),'Attack Rate (%)',"population_group",c(-5,30))
p4_secondary <- forest_plot_zika(d4|> filter(qa_score>0.5) |> filter(arate=='Secondary'),'Secondary Attack Rate (%)',"population_group",c(-5,100))

p4_primary + p4_secondary + plot_annotation(tag_levels = 'A')

# Growth rate
d5$parameter_unit <- 'Per day'
p5 <- forest_plot_zika(d5|> filter(qa_score>0.5) ,'Growth rate (r)',"pathogen",c(0,.25))
...
p6 <- forest_plot_zika(d6,'Reproduction Number',"parameter_type",c(0,10))
p6_1 <- forest_plot_zika( d6 |>
                       filter(qa_score>0.5) |>
