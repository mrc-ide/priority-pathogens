...
# no longer have to do this as it is in the zika_compilation step
...
#          cfr_ifr_denominator = sum(cfr_ifr_denominator),
...
janitor::tabyl(CZS_rate_qa$trimester_exposed, useNA = 'ifany')
max(CZS_rate_qa$central, na.rm=TRUE)
min(CZS_rate_qa$central, na.rm=TRUE)
...
CZSplot <- forest_plot(CZS_rate %>% filter(qa_score >= 0.5) %>% arrange(central) ,
                       label = "CZS probability given ZIKV-infected mother (%)",
...
CZS_rate <- CZS_rate %>%
  # filter out esitmates with n <10
...
length(unique(CZSqa$covidence_id))
CZS_meta_noqa <-  CZS_rate %>%
  filter(metaanalysis_inclusion == 'include')
...
metaanalysis_CZS <- metaprop_wrap(dataframe = CZSqa %>% filter(trimester_exposed %in% c('Unspecified','Whole pregnancy')),
                                  ...
                                  sort_by_subg = FALSE, xlabel = "CZS probability\ngiven ZIKV-infected mother",
...
funnel_czsmeta <- funnel(metaanalysis_CZS$result, common = FALSE,#only plots funnel for either common or random for some reason
                                    pch = 22, bg = "dodgerblue3", level = 0.95)#,
# studlab = TRUE, cex.studlab = 0.75, pos.studlab = 2)
dev.off()
gg <- png::readPNG("temp.png", native = TRUE)
file.remove("temp.png")
funnel_czs <- wrap_elements(plot = rasterGrob(gg, interpolate = TRUE))
ggsave("CZS_funnel.pdf", plot = funnel_czs, width = 8, height = 6)

ggsave(filename = "CZS_metaanalysis.png", plot = CZS_meta, width = 5, height = 5)
ggsave(filename = "CZS_metaanalysis.pdf", plot = CZS_meta, width = 5, height = 5)
...
metaanalysis_CZS_continent <- metaprop_wrap(dataframe = CZSqa %>% filter(!is.na(continent) & trimester_exposed %in% c('Unspecified','Whole pregnancy')),
                                            ...
                                            sort_by_subg = TRUE, xlabel = "CZS probability\ngiven ZIKV-infected mother",
                                            plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                            width = 5000, height = 6600, resolution = 600)
CZS_meta_continent <- metaanalysis_CZS_continent$plot

ggsave(filename = "CZS_metaanalysis_continent.png", plot = CZS_meta_continent, width = 5, height = 6.6)
ggsave(filename = "CZS_metaanalysis_continent.pdf", plot = CZS_meta_continent, width = 5, height = 6.6)

# # Country meta-analysis
# CZS_ratecountry <- CZSqa %>%
...
metaanalysis_CZS_brazil <- metaprop_wrap(dataframe = CZSqa %>%
                                           filter(population_country %in% c('Brazil')),#paste0(refs, ": ", population_location)),
                                         plot_pooled = TRUE, subgroup = 'trimester_exposed',
                                         sort_by_subg = TRUE, xlabel = "CZS probability\ngiven ZIKV-infected mother",
                                         plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                         width = 6000, height = 7000, resolution = 600)
...
ggsave(filename = "CZS_metaanalysis_brazil.pdf", plot = CZS_meta_brazil, width = 6, height = 7)
...
ggsave("CZS_combocountries_funnel.pdf", plot = funnel_czsbrazil  + plot_annotation(tag_levels = 'A'), width = 8, height = 4)#+ funnel_czscolombia + funnel_czsfrance
...
metaanalysis_CZS_trimester_noqa <- metaprop_wrap(dataframe = CZS_meta_noqa,
                                            ...
                                            sort_by_subg = FALSE, xlabel = "CZS probability\ngiven ZIKV-infected mother",
                                            plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                            width = 5000, height = 10000, resolution = 600)
CZS_meta_trimester_noqa <- metaanalysis_CZS_trimester_noqa$plot

ggsave(filename = "CZS_metaanalysis_trimester_noqa.svg", plot = CZS_meta_trimester_noqa, width = 5, height = 9.6)
ggsave(filename = "CZS_metaanalysis_trimester_noqa.pdf", plot = CZS_meta_trimester_noqa, width = 5, height = 9.6)
...
metaanalysis_CZS_noqa <- metaprop_wrap(dataframe = CZSqa %>% filter(trimester_exposed %in% c('Unspecified','Whole pregnancy')) ,
                                       ...
                                       sort_by_subg = FALSE, xlabel = "CZS probability\ngiven ZIKV-infected mother",
                                       plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                       width = 5000, height = 6500, resolution = 600)
CZS_meta_noqapl <- metaanalysis_CZS_noqa$plot

ggsave(filename = "CZS_metaanalysis_noqa.svg", plot = CZS_meta_noqapl, width = 9, height = 21)
ggsave(filename = "CZS_metaanalysis_noqa.pdf", plot = CZS_meta_noqapl, width = 9, height = 21)
...
miscarriage <- parameters %>%
  filter(parameter_type == 'Pregnancy loss probability') %>%
...
max(misc_qa$central, na.rm = TRUE)
min(misc_qa$central, na.rm = TRUE)
...
#                    label = "Pregnancy loss probability (%)",
...
metaanalysis_misc <- metaprop_wrap(dataframe = miscarriagemeta,
                                   plot_pooled = TRUE, subgroup = NA,
                                   sort_by_subg = FALSE, xlabel = "Pregnancy loss probability\ngiven ZIKV-infected mother",
                                   plot_study = TRUE, digits = 4, colour = "dodgerblue3",
...
#Brazil
metaanalysis_misc_brazil <- metaprop_wrap(dataframe = miscarriagemeta %>%
                                           filter(population_country %in% c('Brazil')),#paste0(refs, ": ", population_location)),
                                         plot_pooled = TRUE, subgroup = 'trimester_exposed',
                                         sort_by_subg = TRUE, xlabel = "Pregnancy loss probability\ngiven ZIKV-infected mother",
                                         plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                         width = 5000, height = 3000, resolution = 600)
misc_meta_brazil <- metaanalysis_misc_brazil$plot

ggsave(filename = "misc_metaanalysis_brazil.png", plot = misc_meta_brazil, width = 5, height = 4.5)
ggsave(filename = "misc_metaanalysis_brazil.pdf", plot = misc_meta_brazil, width = 5, height = 4.5)
...
metaanalysis_pregoutcome <- metaprop_wrap(dataframe = miscarriagemeta %>%
                                            mutate(pregnancy_outcome_type = case_when(pregnancy_outcome_type == 'miscarriage'  ~ 'Miscarriage',
                                                                                      pregnancy_outcome_type == 'general pregnancy loss' ~ 'General pregnancy loss',
                                                                                      TRUE ~ pregnancy_outcome_type)),
                                         plot_pooled = TRUE, subgroup = 'pregnancy_outcome_type',
                                         sort_by_subg = FALSE, xlabel = "Pregnancy loss probability\ngiven ZIKV-infected mother",
                                         plot_study = TRUE, digits = 4, colour = "dodgerblue3",
                                         width = 5000, height = 5000, resolution = 600)
misc_meta_pregoutcome <- metaanalysis_pregoutcome$plot

ggsave("misc_metaanalysis_pregoutcome.png", plot = misc_meta_pregoutcome, width = 5, height = 5.5)
ggsave("misc_metaanalysis_pregoutcome.pdf", plot = misc_meta_pregoutcome, width = 5, height = 5.5)
...
AAABBBB
AAABBBB"
CZS_plot_combined <- CZSplot + CZS_meta_trimester +
...
CZS_plot_combined_noqa <- CZSplot_noqa + CZS_meta_trimester_noqa +
  plot_layout(design = layout) #+ plot_annotation(tag_levels = 'A')
ggsave(filename = "CZS_plot_combined_noqa.png", plot = CZS_plot_combined_noqa, width = 35, height = 32, bg = 'white', dpi = 600)
ggsave(filename = "CZS_plot_combined_noqa.pdf", plot = CZS_plot_combined_noqa, width = 35, height = 32, bg = 'white', dpi = 600)
