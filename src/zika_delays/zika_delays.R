...
onset_recovery_forest <- forest_plot_zika(d4 %>% filter(qa_score>=0.5)|> arrange(desc(parameter_value)),
                                     ycol = 'label_group',
...
# Serial interval
SI_forest <- forest_plot_zika(d5 %>% filter(qa_score>=0.5)|> arrange(desc(parameter_value)),
                         ycol = 'label_group',
...
# Infectious period
IP_forest <- forest_plot_zika(d6 %>% filter(qa_score>=0.5)|> arrange(desc(parameter_value)),
                         ycol = 'label_group',
...
IP_forest_noqa <- forest_plot_zika(d6 |> arrange(desc(parameter_value)),
                              ycol = 'label_group',
                              label = 'Infectious Period (days)',
...
# Generation time
# GT_forest <- forest_plot_zika(d7 %>% filter(qa_score>0.5)|> arrange(desc(parameter_value)),
...
# Incubation period
# incp1 <- forest_plot_zika(d1 %>% filter(qa_score>0.5) |> arrange(parameter_value,desc(parameter_value)),
...
incp2 <- forest_plot_zika(d1 %>% filter(qa_score>=0.5) |> arrange(parameter_value,desc(parameter_value)),
...
# incp1_noqa <- forest_plot_zika(d1  |> arrange(parameter_value,desc(parameter_value)),
#                      label = 'Incubation Period (days)',
#                      ycol = 'label_group',
#                      color_column = "population_sample_type",
#                      shape_column = "parameter_value_type",
...
incp2_noqa <- forest_plot_zika(d1 |> arrange(parameter_value,desc(parameter_value)),
                     label = 'Intrinsic incubation Period (days)',
                     ycol = 'label_group',
                     color_column = "population_group", # had these backwards initially
                     shape_column = "population_sample_type",
...
lat_onset_serialplt <- forest_plot_zika(lat_onset_serial_admission %>% filter(qa_score >= 0.5) |> arrange(parameter_value,desc(parameter_value)),
            label = 'Delay in days',
            ycol = 'label_group',
            shape_column = "population_sample_type",
            color_column = "population_group",
...
lat_onset_serialplt_noqa <- forest_plot_zika(lat_onset_serial_admission |> arrange(parameter_value,desc(parameter_value)),
                                   label = 'Delay in days',
                                   ycol = 'label_group',
                                   shape_column = "population_sample_type",
                                   color_column = "population_group",
...
# latent period
p2 <- forest_plot_zika(d2 %>% mutate(parameter_unit = 'Days'),
...
outcome_forest <- forest_plot_zika(d3 %>% filter(qa_score>=0.5) |> arrange(parameter_type,desc(parameter_value)) %>%
...
outcome_forest_noqa <- forest_plot_zika(d3  |> arrange(parameter_type,desc(parameter_value)) %>%
                                     mutate(parameter_type = stringr::str_to_title(str_replace(parameter_type,'Human delay - ',''))),
                                   ycol = 'label_group',
                                   label = 'Admission to discharge/recovery (days)',
                                   color_column = "population_group", # had these backwards initially
...
outcome_forest_noqa_type <- forest_plot_zika(d3  |> arrange(parameter_type,desc(parameter_value)) %>%
...
# Mosquito delay - EIP
eip_forest <- forest_plot_zika(m1 %>% filter(qa_score>=0.5)|> arrange(population_sample_type,desc(parameter_value)),
...
eip_forest_noqa <- forest_plot_zika(m1 |> arrange(population_sample_type,desc(parameter_value)),
                          label = 'Extrinsic incubation period (days)',
                          ycol = 'label_group',
                          # shape_column = 'popultio',
                          color_column = 'refs',
