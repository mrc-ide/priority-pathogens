...
parameters <- parameters %>% mutate(parameter_unit = case_when(
  ...
  parameter_unit == 'Unspecified' & parameter_type %in% c("Miscarriage proportion", "Zika congenital syndrome (microcephaly) proportion") ~ '',
...
# parameters -- miscarriage proportion/microcephaly proportion ----
infants <- parameters %>%
  filter(parameter_type %in% c("Miscarriage proportion", "Zika congenital syndrome (microcephaly) proportion")) %>%
  select(parameter_type, parameter_value, unc_type, #uncertainty,
         cfr_ifr_method, cfr_ifr_numerator,cfr_ifr_denominator,
         population_country, label_group, dates,
