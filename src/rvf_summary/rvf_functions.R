  ...
  param4plot <- parameters |>
    ...
    mutate(across(
      c(parameter_2_value,
        ...
        #parameter_2_sample_paired_upper, #range on uncertainty, not used anymore for MERS
...
forest_plot <- function(df, label, color_column, lims, text_size = 11,
  ...
  if (all(df$parameter_class=="Reproduction number")) {
    gg <- gg +
      geom_vline(xintercept = 1,linetype = "solid", colour = "dark grey")
  }
...
insert_blank_rows <- function(dataframe, column){
  ...
  if(is.null(levels(dataframe[[column]]))){
    tlabels <- as.character(unique(dataframe[[column]]))
  }else{
    tlabels <- as.character(levels(dataframe[[column]]))
  }
  
  df_split <- split(dataframe, dataframe[[column]])|> 
    lapply(function(group) {
      # add blank row
      df <- rbind(group, rep(NA, ncol(group)))  
      # match column names
      colnames(df) <- colnames(dataframe)
      return(df)
    })
  
  dataframe <- do.call(rbind, df_split)
