...
orderly_artefact(description = "MERS supplementary figures",
...
#table(articles$year_publication)
...
# max(table(quality$score))
# 41 is the max article count, but 57 is the max count within a bin of width 10
p2 <- ggplot() +
...
p2 <- models |>
  ...
  scale_x_discrete(
    labels = c("Airborne or close contact,Human to human (direct contact)" = "Airborne & \n Human to human",
               "Airborne or close contact,Vector/Animal to human" = "Airborne & \n Animal to human",
...
#TODO: I'm going to assume there's more that we're missing here?
parameters <- parameters |>
  mutate(
    parameter_type = case_when(
      ...
      parameter_type == "Human delay - infectious period"~"Infectious period",
...
parameters <-parameters |>
  mutate(population_study_end_year=ifelse(population_study_end_year=="xxxx",
         ...
         population_study_start_year=ifelse(population_study_start_year=="xxxx",
                                          NA,
