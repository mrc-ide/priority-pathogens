...
source(file.path('..', 'src','zika_prep_data','data_curation.R'))
...
dfs <- data_curation_zika(articles, outbreaks,models,parameters, plotting =  TRUE, switch_first_surname = TRUE )
...
```{r Figure_2B, echo=FALSE, fig.height= 7, fig.width=15}
final_plot
```
...
r0_sampletype <- forest_plot_zika(df %>% filter(qa_score >= 0.5 & parameter_type == 'Reproduction number (Basic R0)') %>%
