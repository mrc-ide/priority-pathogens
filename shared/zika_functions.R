...
forest_plot_zika <- function(df, ycol = "urefs", label, shape_column = 'parameter_value_type',
  ...
  df   <- df %>% mutate(urefs = make.unique(refs)) %>%
    mutate(urefs = factor(urefs, levels = rev(unique(urefs))),
           ...
           continent = factor(continent, levels = c('Brazil (Americas)','French Polynesia, France (Oceania)','Colombia (Americas)', 'Americas','Asia','Europe','Oceania','Africa',"Multiple continents","Unspecified")),
  ...
  } else if (facet_by_trimester){
    gg <- gg +
      ggforce::facet_col(vars(trimester_exposed), scales = 'free_y', space = 'free')
  }
